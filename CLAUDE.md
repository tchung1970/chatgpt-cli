...
Interactive commands: `/quit` (or `q` / `exit`) to quit, `/clear` to reset conversation, `/model` to show model, `/paste` for multi-line input mode (end with `/end`), `/help` for help.
